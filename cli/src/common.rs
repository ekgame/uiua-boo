...
fn validate_piece(name: &str, piece: &str) -> Result<(), String> {
    ...
    if name.chars().any(|c| !c.is_ascii_alphanumeric() && c != '-') {
        return Err(format!("{} can only contain alphanumeric characters and dashes.", piece));
    }
    ...
    if length < min_length || length > max_length {
        return Err(format!("{} must be between {} and {} characters long.", piece, min_length, max_length));
    }
